...
})()}).call(this);(function($){$(window).load(function(){initConnectionSettings()});function initConnectionSettings(){var cs={apiKey:"AIzaSyAboyEVKy1dJXbDzSn2FH72eewl1wA2Z80",authDomain:"table-champ.firebaseapp.com",databaseURL:"https://table-champ.firebaseio.com"};$(".login").html(tmpl("loginForm",{email:i18n.index.loginForm.email,password:i18n.index.loginForm.password,button:i18n.index.loginForm.button})).show();loadFirebaseSettings(cs)}function init(){try{initLoginForm()}catch(err){console.log(err)}}function initLoginForm(){var auth=firebase.auth(),database=firebase.database();$(".login form").on("submit",function(){var email=$('input[name="email"]').val(),password=$('input[name="password"]').val();auth.signInWithEmailAndPassword(email,password).catch(function(error){var errorCode=error.code;var errorMessage=error.message;console.log(errorMessage);$(".errors").text(errorMessage).addClass("show")});return false});auth.onAuthStateChanged(function(user){if(user){window.location="./app.html"}})}function loadFirebaseSettings(cs){if(typeof firebase!=="undefined"){var config={apiKey:cs.apiKey,authDomain:cs.authDomain,databaseURL:cs.databaseURL};firebase.initializeApp(config);init()}}})(jQuery);